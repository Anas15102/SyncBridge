    ...
    var body: some View {
        NavigationSplitView {
            List(selection: $selectedTab) {
                NavigationLink(value: "SyncBridge") {
        ...
        VStack {
            Form {
                ...
                HStack(spacing: 30) {
                    Spacer(minLength: 0)
                    Button {
                        if let url = URL(string: "https://github.com/kde/kdeconnect-kde") {
            ...
            VStack(spacing: 0) {
                Divider()
                Text("SyncBridge • Cross-Device AirDrop & Clipboard Sync")
                    .foregroundStyle(.secondary)
                    .padding(.top, 5)
                    .padding(.bottom, 7)
                    .multilineTextAlignment(.center)
...
struct SyncBridgeDevicesSettings: View {
    ...
    var body: some View {
        Form {
            Section {
                Toggle("Live Clipboard Sync", isOn: $appState.isClipboardSyncEnabled)
                HStack {
                    Text("Device ID")
                    Spacer()
                    Text(appState.localIdentity.deviceId)
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
                HStack {
                    ...
                    Spacer()
                    Text("v\(appState.localIdentity.protocolVersion)")
                        .foregroundColor(.secondary)
                }
            } header: {
                Text("SyncBridge Engine")
            }

            Section {
                HStack {
                    TextField("Enter phone IP (e.g. 192.168.0.30)", text: $directIp)
                        .textFieldStyle(.roundedBorder)
                    Button("Connect") {
                        if !directIp.isEmpty {
                            appState.connectTo(ipAddress: directIp)
                        }
                    }
                }
                Button(action: {
                    ...
                    Label("Scan / Ping Network", systemImage: "arrow.clockwise")
                }
            } header: {
            ...
            Section {
                if appState.devices.isEmpty {
                    Text("Searching for Android devices on Wi-Fi...")
                        .foregroundColor(.secondary)
                } else {
                    ForEach(appState.devices) { device in
                        HStack(spacing: 12) {
                            Image(systemName: device.type == "phone" ? "iphone" : "laptopcomputer")
                                .font(.title2)
                                .foregroundColor(device.isReachable ? .green : .gray)
                            VStack(alignment: .leading, spacing: 2) {
                                HStack(spacing: 6) {
                                    Text(device.name)
                                        .font(.headline)
                                ...
                                Text("\(device.ipAddress) • \(device.pairingState.rawValue.capitalized)")
                                    .font(.caption)
                                    .foregroundColor(.secondary)
                            }
                            Spacer()
                            if device.pairingState == .paired {
                                ...
                                Button("Unpair") {
                                    appState.unpair(device: device)
                                }
                                .buttonStyle(.bordered)
                                .controlSize(.small)
                            } else {
                                Button("Pair") {
                                    appState.pair(device: device)
                                }
                                .buttonStyle(.borderedProminent)
                                .controlSize(.small)
                            }
                ...
                Text("Connected KDE Connect Devices (\(appState.devices.count))")
            } footer: {
                Text("Drop files onto your screen's notch to beam them directly to your paired phone.")
            }
        }
        .accentColor(.effectiveAccent)
