...
struct DynamicNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("SyncBridge Notch", systemImage: "sparkle", isInserted: $showMenuBarIcon) {
            Button("Settings & KDE Hub") {
            ...
            Button("Ping Paired Devices") {
                AppState.shared.pingAllPairedDevices()
            }
            Divider()
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? SyncBridgeSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? SyncBridgeSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createSyncBridgeWindow(for screen: NSScreen, with viewModel: BoringViewModel) -> NSWindow {
        let rect = NSRect(x: 0, y: 0, width: windowSize.width, height: windowSize.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]
        
        let window = SyncBridgeSkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !Defaults[.showOnAllDisplays] {
            ...
            let window = createSyncBridgeWindow(
        ...
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    ...
                    let window = createSyncBridgeWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createSyncBridgeWindow(for: selectedScreen, with: vm)
            }
